...
def mix_columns(state):
    # Implement mix columns logic
    return state

def inv_mix_columns(state):
    # Implement inverse mix columns logic
    return state
...
def encrypt_block(state, expanded_key, rounds):
    ...
    for round_idx in range(1, rounds):
        ...
        state = add_round_key(state, expanded_key[round_idx * 16:(round_idx + 1) * 16])
    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, expanded_key[rounds * 16:(rounds + 1) * 16])
    return state
...
def encrypt(message: bytes, key: bytes, rounds=10):
    expanded_key = expand_key(key, rounds)
    state = list(message)
    encrypted_state = encrypt_block(state, expanded_key, rounds)
    return bytes(encrypted_state)

def decrypt(ciphertext: bytes, key: bytes, rounds=10):
    expanded_key = expand_key(key, rounds)
    state = list(ciphertext)
    decrypted_state = decrypt_block(state, expanded_key, rounds)
    return bytes(decrypted_state)
...
message = st.text_input("Message (Hexadecimal)", placeholder="e.g., 00112233445566778899aabbccddeeff")
key = st.text_input("Key (Hexadecimal)", placeholder="e.g., 000102030405060708090a0b0c0d0e0f")
